...
test("real Vault profile startup pins each selected leaf and revokes retained runtime handles at expiry", async () => {
	...
	for (const derivation of candidates) {
		...
		expect(initializeWallet.mock.calls[0]?.[0].toHex()).toBe(expected.toHex());
		...
		await expect(retained.getPublicKey({ identityKey: true })).rejects.toThrow(
			"Unlock",
		);
