...
(() => {
 ...
 let backend = initialBackend, inventory, source, destination, session, preview, state = 'inventory';
 ...
 async function api(path, options = {}) {
  ...
  if (!response.ok) throw new Error(data && data.error ? data.error : 'Local setup request failed');
  return data;
 }
 ...
 function renderSources() {
  ...
  for (const item of inventory.sources) {
   ...
   const button = document.createElement('button'); button.type = 'button'; button.textContent = 'Select'; button.addEventListener('click', () => { source = item; destination = undefined; $('destination-form').reset(); $('unlock').disabled = true; setText('status', 'Source selected: ' + item.account + '. Choose a Vault destination.'); $('choose-destination').disabled = false; }); row.append(button); host.append(row);
  }
 }
 function showFailure(error) { setText('status', error.message); $('status').className = 'error'; show('recovery-section', true); setText('recovery', error.message); }
 ...
 $('lock').addEventListener('click', async () => { try { if (session) await api('/api/migration/lock', {method:'POST', body:JSON.stringify({sessionId:session.sessionId})}); session = undefined; preview = undefined; show('preview-section', false); setText('status', 'Vault session locked. Unlock again locally to continue.'); } catch (error) { showFailure(error); } });
 function renderPreview() { show('preview-section', true); $('confirmation').disabled = false; const safe = {source:preview.source, destination:preview.destination, preservation:preview.preservation, conflicts:preview.conflicts}; $('preview-data').textContent = JSON.stringify(safe, null, 2); const conflicts = $('conflicts'); conflicts.replaceChildren(); if (preview.conflicts.length) { const note = document.createElement('p'); note.className = 'warning'; note.textContent = 'Resolve every existing Vault conflict before cutover.'; conflicts.append(note); for (const conflict of preview.conflicts) { const row = document.createElement('label'); row.textContent = conflict.message; const select = document.createElement('select'); select.innerHTML = '<option value="">Choose a resolution</option><option value="keep-existing">Keep existing Vault entry</option><option value="import-source">Import source material</option><option value="skip">Skip this item</option>'; select.value = conflict.resolution || ''; select.addEventListener('change', async () => { try { const data = await api('/api/migration/resolve', {method:'POST', body:JSON.stringify({sessionId:session && session.sessionId, conflictId:conflict.id, resolution:select.value})}); preview = data.preview; renderPreview(); } catch (error) { showFailure(error); } }); row.append(select); conflicts.append(row); } } else { conflicts.textContent = 'No conflicts reported.'; } $('confirmation').addEventListener('input', () => { $('cutover').disabled = $('confirmation').value !== 'MIGRATE_AND_SWITCH' || preview.conflicts.some(conflict => !conflict.resolution); }); }
 $('cutover').addEventListener('click', async () => { if ($('confirmation').value !== 'MIGRATE_AND_SWITCH' || !session) return; $('cutover').disabled = true; show('progress-section', true); setText('progress', 'Preparing a recoverable backup…'); try { const data = await api('/api/migration/cutover', {method:'POST', body:JSON.stringify({sessionId:session.sessionId, confirmation:'MIGRATE_AND_SWITCH'})}); setText('progress', 'Migration verified. Account ' + data.accountName + ' is ready in Vault.'); $('progress-section').className = 'success'; setText('status', 'Vault cutover complete.'); show('recovery-section', false); } catch (error) { showFailure(error); $('cutover').disabled = false; } });
