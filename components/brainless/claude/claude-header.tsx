 ...
 *
 * Source: https://brainless.swerdlow.dev (claude-header, MIT).
 */
const ROSE = "#cd694a";
const GRAY = "#949494";
	...
	LOGO_BITS.forEach((row, y) => {
		...
		while (x < w) {
			if (row[x] === "1") {
				...
				rects.push(
					<rect
						// biome-ignore lint/suspicious/noArrayIndexKey: static logo sprite, coordinates never reorder.
						key={`${x}-${y}`}
						x={x}
						y={y * PH}
						width={end - x}
